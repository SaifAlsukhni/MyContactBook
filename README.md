...
MyContactBook uses Timestamp and JavaFX ListView to fulfill the content not covered in class requirement.  Timestamp is used to log when a contact is added.  JavaFX ListView is used to display the list of added contacts.  JavaFX TableView is used to display a sortable list of contacts.  JavaFX Alerts were used to notify the user of successful add, update, or delete actions.  An inner class and lambda expressions are used in the controller (ContactsController) for the events, buttons, and search filter.  Lambda expressions are seen buttonAction class that provides funtionality to all the button in the GUI as well as the search class.  Inner class is used for the search functionality within the viewContacts method.  The app can also write the Contact list into a CSV file for sharing.
